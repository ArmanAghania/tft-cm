...
class AgentListView(OrganisorAndLoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for agent in agent_list:
            ...
            monthly_sales = Sale.objects.filter(organisation=organisation, agent=agent, date__date__range=(gregorian_start_of_month, jalali_today.togregorian())).aggregate(Sum('amount'))['amount__sum'] or 0
            total_leads = Lead.objects.filter(organisation=organisation,agent=agent, date_assigned__range=(gregorian_start_of_month, jalali_today.togregorian())).count()
            converted_leads = Sale.objects.filter(organisation=organisation,agent=agent, date__date__range=(gregorian_start_of_month, jalali_today.togregorian())).values('lead').distinct().count()
            total_leads_overall = Lead.objects.filter(organisation=organisation,agent=agent).count()
            converted_leads_overall = Sale.objects.filter(organisation=organisation,agent=agent).values('lead').distinct().count()
